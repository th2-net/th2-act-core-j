...
class TestDSL {
    ...
    private fun messageBuild(messageType: String, sessionAlias: String, direction: Direction, sequence: Long): Message.Builder =
    ...
    fun `send message and wait echo`() {
        ...
        Context(handler, subscriptionManager, messageRouter, eventRouter, parentEventId) `do` {
    ...
    fun `test should receive response message`() {
        ...
        Context(handler, subscriptionManager, messageRouter, eventRouter, parentEventId) `do` {
            ...
            assertEquals(messages[0], receive("NewOrderSingle", "sessionAlias", Direction.FIRST, 2_000) {
    ...
    fun `case one`() {
        ...
        var sequence = 1L
        while (sequence < 4L){
            messages.add(messageBuild(TestMessageType.QUOTE_STATUS_REPORT.typeName, "anotherSessionAlias", Direction.FIRST, sequence)
    ...
    fun `case two`() {
        ...
        for (it in 3L downTo 1L){
            messages.add(messageBuild(TestMessageType.NEW_ORDER_SINGLE.typeName, "sessionAlias", Direction.FIRST, it).build())
        }
        ...
        Context(handler, subscriptionManager, messageRouter, eventRouter, parentEventId ) `do` {
            send(messageBuild(TestMessageType.NEW_ORDER_SINGLE.typeName, "sessionAlias", Direction.FIRST, 4L).build(), "sessionAlias")
            ...
            do {
                ...
                val responseDQ126 = receive(TestMessageType.DQ126.typeName,"sessionAlias", Direction.FIRST,2_000) {
                    passOn(TestMessageType.DQ126.typeName) {
                        this.messageType == messageDQ126.messageType
                    }
        ...
        Context(handler, subscriptionManager, messageRouter, eventRouter, parentEventId) `do` {
            ...
            resultBuilder.setSingleMessage(
                receive(TestMessageType.BUSINESS_MESSAGE_REJECT.typeName, "sessionAlias", Direction.FIRST, 2_000) {
                    passOn(TestMessageType.BUSINESS_MESSAGE_REJECT.typeName) {
                        this.sequence == 1L
                    }
                    failOn(TestMessageType.BUSINESS_MESSAGE_REJECT.typeName) {
                        this.sequence == 2L
                    }
                }
            )
        ...
        Context(handler, subscriptionManager, messageRouter, eventRouter, parentEventId) `do` {
            ...
            resultBuilder.setSingleMessage(
                receive(TestMessageType.ORDER_CANCEL_REJECT.typeName, "sessionAlias", Direction.FIRST, 2_000) {
                    passOn(TestMessageType.ORDER_CANCEL_REJECT.typeName) {
