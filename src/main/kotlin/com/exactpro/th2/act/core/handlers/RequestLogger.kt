...
class RequestLogger(private val logger: KLogger): RequestHandler() {

    override fun handle(request: IRequest, responder: IResponder, requestContext: RequestContext) {

        logger.info { "Received ${requestContext.requestName} for ${requestContext.rpcName}: ${request.debugString}" }
